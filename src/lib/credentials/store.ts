...
const StoredCredentials = Schema.parseJson(Schema.Struct({ url: Schema.String, key: Schema.String }))
...
export const makeCredentialsStore = Effect.gen(function* () {
  ...
  const setCredentials = (url: string, key: string) =>
    Effect.gen(function* () {
      yield* store.update(() => ({ status: "known", credentials: { url, key } }))
      yield* Effect.sync(() => localStorage.setItem(STORAGE_KEY, JSON.stringify({ url, key })))
    })
